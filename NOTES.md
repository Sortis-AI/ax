...
- **Machine ID instability (containers)**: Both `tokens.json` and `pending_auth.json` are AES-256-GCM encrypted using a key derived from `/etc/machine-id`. If `/etc/machine-id` changes between `ax auth login` and `ax auth callback` — which happens in containerized environments that regenerate machine IDs on each run — decryption fails with "Failed to decrypt pending auth (machine ID changed?)". Agents running in ephemeral containers must be given a stable, persistent `/etc/machine-id` (or use OAuth 1.0a / Bearer token auth instead). There is no fallback or recovery path; the fix is environment-level.
